...
const string PDFName=Form("../PDF/plot_H2Emu_2ndOrder_%s.pdf", Tag.c_str());
...
int plot_2ndorder(void){
  ...
  for(int j=0;j<3;j++){
    ...
    if(j==0){
      ...
      hFrame = new TH2D("hFrame", ";Time;Electric Energy Density", 50, -0.05, 1.05, 50, 0.0, 2.5);
    }
    hFrame->GetYaxis()->SetNdivisions(408);
